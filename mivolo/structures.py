...
class PersonAndFaceResult:
    ...
    def get_distance_to_center(self, bbox_ind: int) -> float:
        ...
        """
        im_h, im_w = self.yolo_results[bbox_ind].orig_shape  # not used in our case, so not fixed
    ...
    def crop_object(
        ...
        other_bboxes: List[torch.tensor] = [
            self.get_bbox_by_ind(other_ind, *full_image.shape[:2]) for other_ind in range(len(self.boxes))
        ]
